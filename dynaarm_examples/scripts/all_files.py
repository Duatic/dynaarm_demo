...
import math
import pandas as pd
import glob  # For getting all files in directory
...
class DynaArm(Node):
    def __init__(self):
        ...
        self.trajectory_dir = os.path.join(package_path, "scripts")

        # Get all trajectory files
        self.trajectory_files = sorted(glob.glob(os.path.join(self.trajectory_dir, "trajectory_data_config*.csv")))

        if not self.trajectory_files:
            self.get_logger().error("No trajectory files found!")
            return

        self.get_logger().info(f"Found {len(self.trajectory_files)} trajectory files.")
    ...
    def execute_all_trajectories(self):
        """Loads and executes all trajectory files sequentially with a small delay in between."""

        for i, trajectory_file in enumerate(self.trajectory_files):
            self.get_logger().info(f"Executing trajectory {i+1}/{len(self.trajectory_files)}: {trajectory_file}")

            # Move to q0 before each trajectory
            self.move_to_q0()

            # Load and execute trajectory
            self.move_joints(trajectory_file)

            # Small delay before the next trajectory
            self.get_logger().info("Waiting 2 seconds before next trajectory...")
            time.sleep(2)

        self.get_logger().info("All trajectories executed successfully!")

    def move_joints(self, csv_path):
        """Loads trajectory from a given CSV file and executes it safely."""
